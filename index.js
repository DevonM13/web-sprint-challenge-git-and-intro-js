...
function getArtistByIndex(person, num) {
  return `the artist at index ${person[num]['id']} is ${person[num]['name']}`
}
...
function listOfNames(people) {
  const artistsCopy = [...people];
  for(let i = 0; i < people.length; i++) {
    artistsCopy[i] = people[i]['name'];    
  } 
  return artistsCopy;
}
...
function removeArtist(people, num) {
  const artistsCopy = [...people];
  artistsCopy.splice(num, 1);
  return artistsCopy;  
}
...
function addArtist(people, name, years, genre, nationality, bio) {
  const artistsCopy = [...people];
  const newObj = { 
    name: name,
    years: years,
...
function lotsOfArt(people) {
  const filteredArray = [];
  for(let i = 0; i < people.length; i++) {
    if(people[i]['paintings'] > 100) {
      filteredArray.push(people[i]['name'])
    }
  }
...
function artistInfo(people, artistsName) {
  for(let i = 0; i < people.length; i++) {
    if(people[i]['name'] === artistsName) {
        return people[i]['bio'];
    }
  }  
}
...
function artistByCountry(people, nationality) {
  const filteredArray = [];
  for(let i = 0; i < people.length; i++) {
    if(people[i]['nationality'] === nationality) {
      filteredArray.push(people[i]['name']);
    }
  }
  return filteredArray;
